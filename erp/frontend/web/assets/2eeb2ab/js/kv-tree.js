...
(function($) {
    ...
    TreeView.prototype = {
        ...
        renderForm: function(key, par, mesg,action) {
       
            var self = this,
                $detail = self.$detail,
                parent = par || '',
                msg = mesg || false,
                params = $h.hashString(key + self.modelClass + self.isAdmin + parent),
                $form = $detail.find('form'),
                vUrl = action!==undefined ? action : self.actions.manage,
                sep = vUrl && vUrl.indexOf('?') !== -1 ? '&' : '?';
        ...
        parseParentFlag: function(key) {
            var self = this,
                $flags = self.$detail.find('input[class="kv-parent-flag"]'),
                $div,
        ...
        remove: function() {
            var self = this,
                $nodeText = self.$tree.find('li .kv-node-detail.kv-focussed'),
                $node = $nodeText.closest('li'),
                msg = self.messages,
                $detail = self.$detail,
                $form = $detail.find('form'),
                $alert, clearNode;
        ...
        move: function(dir) {
            var self = this,
                $nodeText = self.$tree.find('li .kv-node-detail.kv-focussed'),
                $nodeFrom = $nodeText.closest('li'),
                msg = self.messages,
                $detail = self.$detail,
                $nodeTo = null,
                keyFrom, keyTo, outMsg, isRoot = false,
                $parent, fnMove = function() {};
        ...
        create: function() {
            ...
            self.renderForm(null, $node.data('key'),null,self.actions.create);
        ...
        createRoot: function() {
            var self = this,
                $treeRoot = self.$tree.find('.kv-tree'),
            ...
            self.renderForm(null, self.rootKey,null,self.actions.create);
        ...
        check: function($chk) {
            ...
            var self = this,
                isRoot = ($chk === true),
                desc, $node = isRoot ? self.$tree : $chk.closest('li'),
        ...
        clearSearchResults: function() {
            ...
            self.$tree.find('.kv-node-label .kv-search-found').each(function() {
                var $item = $(this),
                    $tmp = $(document.createElement('span')).appendTo($item);
                $tmp.unwrap().remove();
            });
        },
        listen: function() {
