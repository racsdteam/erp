...
/*--------------------------spacing radio options------------------------------------------------*/
 ...
 <?php $form = ActiveForm::begin(['id'=>'pay-create-form', 'action'=>['payrolls/create']]); ?>
 ...
 <div class="row">
  ...
  <?= $form->field($model,  'pay_group')->dropDownList(ArrayHelper::map(PayGroups::find()->all(), 'code', 'name'), ['prompt'=>'Select payroll group',
   ...
   <div class="card-header pl-0">
                            <h3 class="card-title"><i class="fas fa-cog"></i> Payroll Run  Options</h3>
                       </div>     
   
    <?= $form->field($modelSettings, 'copy')->checkbox(['id'=>'checkCopy'])
			->label('Copy Data From Previous Payroll'); ?> 
			
<div id="divCopy">
    
 <?= $form->field($modelSettings, 'prevPayroll')->dropDownList([], ['prompt'=>'Select Previous Payroll To Copy',
               'id'=>'prev','class'=>['form-control  m-select2 ']])->label("Previous Payroll To Copy") ?>     
     ...
     <div class="form-group text-right">
        <?= Html::submitButton('<i class="fas fa-calculator"></i> Save & Process', ['class' =>'btn btn-info']) ?>
    </div>    
