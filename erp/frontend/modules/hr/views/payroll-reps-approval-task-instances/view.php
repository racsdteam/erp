...
$this->title =$model->wfStep0->name;
$this->params['breadcrumbs'][] = ['label' => 'Payroll Reports Approvals', 'url' => ['pending']];
...
?>   
    <div class="row mb-2 action-bar ">
       ...
       data-url="<?= Url::to(["payroll-reps-approval-task-instances/complete"])?>" style="margin-right: 5px;">
...
?>  
    ...
    <?php
         ...
         $firstReport = \yii\helpers\ArrayHelper::filter($model->approvalRequest->getRunReports(), [0]);
        
         $content1=$this->render($firstReport[0]->views['pdf'], [
            'model' => $firstReport[0],'wf'=>!empty($model->wfInst)?$model->wfInst->id : null,
        ]);
        
        $this->context->params['bdc-hide'] =true;
    
  echo  $this->render('@frontend/modules/hr/views/payroll-reps-approval-requests/view',['model' => $model->approvalRequest,
          'content1'=>$content1,'wf'=>!empty($model->wfInst)?$model->wfInst : null]); 
