...
<div class="row clearfix">
  ...
  <?= $form->field($model,  'rpt_type')->dropDownList($reportCategList, ['prompt'=>'Select Report Type',
             ...
             <?= $form->field($model->modelParams, 'paye_basis')->inline()->radioList(['SAL'=>'Salary','ALLOW'=>'Allowance','SUP'=>'Supplemental'],[
            ...
            <div class="rpt-params BL">
             ...
             <?= $form->field($model->modelParams, 'pay_type')->inline()->radioList(['SAL'=>'Salary','ALW'=>'Allowance','LPSM'=>'LumpSum','BON'=>'Bonus'],[
 ...
 <?php
    ...
    echo $form->field($model->modelParams, 'pay_group')->widget(DualListbox::className(),[
...
?>
      ...
      <div class="row">
           ...
           <?= $form->field($model->modelParams, 'period_year',['template' => '
                          {label}
          ...
          <div class="col-sm-12 col-xs-12 col-md-6 col-lg-6">
             ...
             <?= $form->field($model->modelParams, 'period_month',['template' => '
                          {label}
                       <div class="input-group col-sm-12">
                        <div class="input-group-prepend">
...
function toggleInputs(optValue){
 ...
 $('.rpt-params').not('.'+ optValue).hide().find('input').prop("disabled",true);
 
 $('.'+ optValue).show().find('input').prop("disabled",false);
...
//------paye report selected
function payeReportSelect (attribute, value) {
        return $('input:radio[name="PayrollApprovalReports[rpt_type]"]:checked').val()=='PAYE';
	};
	
//------bank list report selected
function bankListReportSelect (attribute, value) {
        return $('input:radio[name="PayrollApprovalReports[rpt_type]"]:checked').val()=='BL';
	};
