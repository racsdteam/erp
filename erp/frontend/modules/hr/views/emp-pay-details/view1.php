         ...
         <?php
               ...
               $payTmpl=$payGrp->payTemplate;
                ...
                //--------------------regular deductions--------------------------------
                 ...
                 $vars=array();
                 $vars[$payTmpl->basePay()->code]=$model->base_pay;
    ...
    'vAlign'=>'middle',
     ...
     <p>
            ...
            <ul class="nav nav-tabs" id="custom-content-above-tab" role="tablist">
              <li class="nav-item">
                ...
                href="#custom-content-above-reg" role="tab" aria-controls="custom-content-above-reg" aria-selected="true">
                   <i class="fas fa-money-bill-wave"></i> Regular Pay</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" id="custom-content-above-sup-tab" data-toggle="pill" href="#custom-content-above-sup" 
                role="tab" aria-controls="custom-content-above-sup" aria-selected="false"><i class="fas fa-money-bill-wave"></i> Additional Pay</a>
              </li>
            ...
            <div class="tab-content" id="custom-content-above-tabContent">
               ...
               <div class="table-responsive">
                  ...
                  <input type="checkbox" name="my-checkbox" data-params='<?php echo json_encode($data) ?>'  data-url=<?=Url::to(['pay-templates/pay-exclude'])?>
                   data-bootstrap-switch data-off-color="danger" data-on-color="success" <?php echo $exempted ?'': 'checked' ;?> >
                    ...
                    <td><?= sprintf("%s (%s)",$payItem->name,$payItem->code)?></td>
                     ...
                     <td><?php 
                     echo !empty( $eLine->formula) ? $eLine->formula  : ''; ?>
                     </td>
                    
                     <td><?php 
                      
                    $amount= ($eLine->payItem->category=='BASE') ? decimalFormat($model->base_pay) : decimalFormat($eLine->amount);
                    echo number_format($amount);
                     ?>
                     </td>
                       <td><?php if($overriden){echo $override->amount;} ?></td>
                      </tr>
                      <?php endforeach ?>
                  ...
                  <input type="checkbox" name="reg-exempt" data-params='<?php echo json_encode($data) ?>' data-url=<?=Url::to(['pay-templates/pay-exclude'])?> 
                   data-bootstrap-switch data-off-color="danger" data-on-color="success" <?php echo $exempted ? '':'checked' ?>>  
                    ...
                    <td><?php 
                     echo $dLine->calc_type=='formula' ? $dLine->formula : ''; ?>
                     </td>
                    
                     <td><?php 
                   
                    
                     echo number_format(decimalFormat($dLine->amount));  
              ...
              <div class="tab-pane fade" id="custom-content-above-sup" role="tabpanel" aria-labelledby="custom-content-above-sup-tab">
             ...
             <div class="table-responsive">
                  <table id="tbl-ed"  class="table">
                    ...
                    <tbody>
                    <tr>
                        <td class="text-muted" colspan="4"><b><i class="ionicons ion-ios-plus-outline"></i> Additional Earnings</b>
                    ...
                    <?php foreach($payTmpl->supEarningsLines()  as $eLine):  ?> 
                    <?php
                    ...
                    <td><?php
                     ...
                     echo sprintf("%s (%s)",$payItem->name,$payItem->code)." ".
                        Html::a('<i class="fas fa-edit"></i>', !empty($override)?  Url::to(['emp-pay-overrides/update','id'=>$override->id]):
                     Url::to(['emp-pay-overrides/create','tmpl'=>$payTmpl->id,'id'=>$eLine->id,'emp_pay'=>$model->id]), 
                     ['class'=>['text-sm text-success action-modal'],
                    ...
                    ?>
                     ...
                     <td><?php 
                    $amount=!empty($override) ? decimalFormat($override->amount) : (!empty($eLine->amount) ? decimalFormat($eLine->amount) : 0);
                    ...
                    echo number_format($amount);
                     ?>
                      ...
                      <!----------------------------deductions------------------------------------------->
                      <tr>
                        <td colspan="4" class="text-muted"><b><i class="ionicons ion-ios-minus-outline"></i> Additional Deductions</b></td>
                      <?php for($i=1 ;$i<4;$i++){ ?>
                        <td style="display: none;"></td>
                        <?php }?>
                   
                    </tr>      
                    <?php foreach( $payTmpl->supDeductionsLines() as $dLine):  ?> 
                    <?php 
                    ...
                    <td>
                        <?php
                          echo sprintf("%s (%s)",$payItem->name,$payItem->code);
                     echo Html::a('<i class="fas fa-pencil-alt"></i>',!empty($override)?  Url::to(['emp-pay-overrides/update','id'=>$override->id]):
                     Url::to(['emp-pay-overrides/create','tmpl'=>$payTmpl->id,'id'=>$eLine->id,'emp_pay'=>$model->id]), 
                     ['class'=>['text-sm text-sucess action-modal'],
