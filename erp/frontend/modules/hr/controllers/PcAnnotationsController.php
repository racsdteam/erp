...
class PcAnnotationsController extends \yii\web\Controller
{
       ...
       if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         ...
         $model->save(false);
           ...
           Yii::$app->db4->createCommand()
                      ->update('pc_annotations', ['annotation' => $data['xfdfString']], ['annotation_id' =>$data['annotationId']])
                      ->execute();  
         ...
         Yii::$app->db4
->createCommand()
->delete('pc_annotations', ['AND', ['doc' => $_REQUEST['documentId']],  ['annotation_id' =>$data['annotationId']]])
->execute();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
          ...
          $q=" SELECT annotation_id,annotation FROM  pc_annotations  where doc='{$_REQUEST['documentId']}'";
    ...
    {
        if (($model = PcAnnotations::findOne($id)) !== null) {
            return $model;
        }
