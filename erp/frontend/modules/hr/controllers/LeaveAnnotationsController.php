...
use frontend\modules\hr\models\LeaveAnnotations;
...
class LeaveAnnotationsController extends \yii\web\Controller
{
       ...
       if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         ...
         $model->save(false);
           ...
           Yii::$app->db4->createCommand()
                      ->update('leave_annotations', ['annotation' => $data['xfdfString']], ['annotation_id' =>$data['annotationId']])
                      ->execute();  
         ...
         Yii::$app->db4
->createCommand()
->delete('leave_annotations', ['AND', ['doc' => $_REQUEST['documentId']],  ['annotation_id' =>$data['annotationId']]])
->execute();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
          ...
          $q=" SELECT annotation_id,annotation FROM  leave_annotations  where doc='{$_REQUEST['documentId']}'";
    ...
    {
        if (($model = LeaveAnnotations::findOne($id)) !== null) {
            return $model;
        }
