 ...
 */
class PcApprovalRequestInstances  extends ApprovalWorkflowInstances
{
    
    const ENTITY_TYPE = 'PerformanceContract';
    
    const TASK_CLASS='frontend\modules\hr\models\PcApprovalTaskInstances';
    const CMT_CLASS='frontend\modules\hr\models\PcApprovalComments';
    const TASK_SBM_EMAIL='pcSubmit-html';
   ...
   public function returnForEdit($varMap) {
    ...
    if($varMap['return_step_id']==0) {
     ...
     $varMap['taskClass']='frontend\modules\hr\models\PcApprovalTaskInstances';
...
{
    return $this->hasOne(PerformanceContract::className(), ['id' =>'entity_record']);
}

  public function getComments()
{
    
    return $this->hasMany(PcApprovalComments ::className(),['wfInstance' => 'id'])->orderBy(['timestamp'=>SORT_DESC]) ;
}
