...
use frontend\modules\hr\models\ApprovalWorkflowInstanceSteps;
 ...
 */
class ProcurementPlansApprovalInstances  extends ApprovalWorkflowInstances
{
    
    const ENTITY_TYPE = 'ProcurementPlans';
    const TASK_CLASS='frontend\modules\procurement\models\ProcurementPlanApprovals';
    const CMT_CLASS='frontend\modules\procurement\models\ProcurementPlanApprovalComments';
    const TASK_SBM_EMAIL='procurementPlanSubmit-html';
    //const PAY_DC_SBM_EMAIL='payrollDCSubmit-html';
    public $errors=[];
       ...
       return $res;  
         ...
         if($res['status']!='success'){
           return $res;
            ...
            $this->entityRecord->trigger(ProcurementPlans::EVENT_REQUEST_APPROVAL);
            }
   ...
   public function returnForEdit($varMap) {
    ...
    if($varMap['return_step_id']==0) {
     ...
     $varMap['taskClass']='frontend\modules\hr\models\PayrollApprovalRequestInstances';
   ...
   protected function complete($outcome){
    ...
    $entityRecord->trigger(ProcurementPlans::EVENT_REQUEST_APPROVAL); 
    }
    $res['status']='success';
    return $res;
...
{
return new \frontend\modules\hr\models\ApprovalWorkflowInstancesQuery(get_called_class(), ['where' =>
...
{
    return $this->hasOne(ProcurementPlans::className(), ['id' =>'entity_record']);
}

  public function getApprovalComments()
{
    return $this->hasMany(ProcurementPlanApprovalComments ::className(),['wfInstance' => 'id'])->orderBy(['timestamp'=>SORT_DESC]) ;
} 
