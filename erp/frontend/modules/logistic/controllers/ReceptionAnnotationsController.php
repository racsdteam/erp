...
use common\models\ReceptionAnnotations;
...
class ReceptionAnnotationsController extends \yii\web\Controller
{
    ...
    {
        $searchModel = new ReceptionAnnotations();
    ...
    public function actionAnnotationsHandler(){
       ...
       if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         ...
         $model=new  ReceptionAnnotations();
         ...
         $model->save(false);
           ...
           Yii::$app->db1->createCommand()
                      ->update('reception_annotations', ['annotation' => $data['xfdfString']], ['annotation_id' =>$data['annotationId']])
                      ->execute();  
         ...
         Yii::$app->db1
->createCommand()
->delete('reception_annotations', ['AND', ['doc' => $_REQUEST['documentId']],  ['annotation_id' =>$data['annotationId']]])
->execute();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
          ...
          $q=" SELECT annotation_id,annotation FROM  reception_annotations  where doc='{$_REQUEST['documentId']}'";
    ...
    {
        if (($model = ReceptionAnnotations::findOne($id)) !== null) {
            return $model;
        }
