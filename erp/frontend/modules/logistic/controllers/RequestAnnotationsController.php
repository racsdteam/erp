...
use common\models\RequestAnnotations;
...
class RequestAnnotationsController extends \yii\web\Controller
{
    ...
    {
        $searchModel = new RequestAnnotations();
       ...
       if ($_SERVER['REQUEST_METHOD'] === 'POST') {
         ...
         $model->save(false);
           ...
           Yii::$app->db1->createCommand()
                      ->update('request_annotations', ['annotation' => $data['xfdfString']], ['annotation_id' =>$data['annotationId']])
                      ->execute();  
         ...
         Yii::$app->db1
->createCommand()
->delete('request_annotations', ['AND', ['doc' => $_REQUEST['documentId']],  ['annotation_id' =>$data['annotationId']]])
->execute();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
          ...
          $q=" SELECT annotation_id,annotation FROM  request_annotations  where doc='{$_REQUEST['documentId']}'";
    ...
    {
        if (($model = RequestAnnotations::findOne($id)) !== null) {
            return $model;
        }
