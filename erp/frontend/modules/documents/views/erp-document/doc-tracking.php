...
<div class="card card-default ">
 <div class="card-header ">
   <h3 class="card-title"><i class="fa fa-binoculars"></i> Document History Tracking</h3>
 </div>
 <div class="card-body">
...
<?php 
     ...
     $q="SELECT f.*  FROM  erp_document_flow_recipients as f where f.document={$id}  order by f.timestamp desc";
			...
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
					...
					$dateParts=dateParts($data['timestamp']);
                      ...
                      $row5=getUserInfo($data['recipient'],$data['timestamp']);
                    $row6=getUserInfo($data['sender'],$data['timestamp']);
                    $row7=getUserSendInfo($data['recipient'],$data['id'],$data['document']);
                   if(empty($row7))
                   {
							...
							<div class="widget-box transparent">
								<div class="widget-header widget-header-small">
									<h5 class="widget-title smaller">
										...
										<span class="grey"> received the Document</span>
									</h5>
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_document_flow_recipients as f where f.id > {$id} and 	f.sender={$user} and f.document={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
                                           return $row;
}
