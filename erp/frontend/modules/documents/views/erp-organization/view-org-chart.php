...
?>
 ...
 <style type="text/css">
    ...
    <div class="row clearfix">
       ...
       <div class="col-sm-12">
        <?php $labels=array('MD'=>'MANAGING DIRECTOR','DMD'=>'DEPUTY MANAGING DIRECTOR',"D"=>"DIRECTORS","M"=>"MANAGERS","A"=>"ADMINISTRATIVE ASSISTANTS","O"=>"OFFICERS");    ?>
        
    <?php foreach($labels as $key=>$label) : ?>
   <?php $class;if($key=='MD'){ $class='label pull-left md';}else if($key=='D'){ $class='label pull-left d';}else if($key=='DMD'){ $class='label pull-left dmd';}
   elseif($key=='M'){$class='label pull-left m';}elseif($key=='A'){$class='label pull-left a';}elseif($key=='O'){$class='label pull-left o';}?>
   <small style="padding:15px;margin-left:10px;border-radius:5px;" class="<?php echo $class?>"><?= $label ?></small> 
   
    
   <?php endforeach; ?> 
  </div> 
</div>
<div id="org-chart-view">
</div>

<?php

$url=Url::to(["erp-organization/get-json"
...
var parents=[];
 ...
 var html='<div class="alert alert-danger alert-dismissable">'+
        ...
        color: "darkred",
                       ...
                       if(value.level=='director' && value.parentId!=null && value.parentId!=1 ){
        ...
        color: "green",
                        ...
                        else if(value.level=='director' && value.id==2  ){
        ...
        color: "purple",
                       ...
                       else if(value.level=='manager'){
                           
                         parents[value.id] = {
        color: "teal",
       
    };  
                       }else if(value.level=='pa'){
                         parents[value.id] = {
        color: "lightgreen",
...
console.log(parents);
                ...
                var peopleElement = document.getElementById("org-chart-view");
            ...
            primaryFields: ["name", "title", "phone", "mail"],
             ...
             //boxSizeInPercentage: { minBoxSize: { width: 2, height: 2 }, boxSize: { width: 30, height: 30 }, maxBoxSize: { width: 30, height: 30 }},
             photoFields: ["image"],
            dataSource:data
