...
<div class="card card-default ">
 <div class="card-header ">
   <h3 class="card-title"><i class="fa fa-binoculars"></i> LPO Request History Tracking</h3>
 </div>
 <div class="card-body">
...
<?php 
   ...
   if (Yii::$app->session->hasFlash('error')){
     ...
     $q="SELECT f.*  FROM erp_lpo_request_approval_flow  as f where f.lpo_request={$id}  order by f.timestamp desc";
			...
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
                    ...
                    $row6=getUserInfo($data['originator'],$data['timestamp']);
                       $row7=getUserSendInfo($data['approver'],$data['id'],$data['lpo_request']);
                   if(empty($row7))
                   {
							...
							<div class="widget-box transparent">
								<div class="widget-header widget-header-small">
									<h5 class="widget-title smaller">
										...
										<span class="grey"> received LPO Request</span>
									</h5>
	                                ...
	                                <h6 class="widget-title smaller">
										...
										<div class="widget-toolbox clearfix">
											...
											<div class="pull-right action-buttons">
											 <?php if(($data['status']!='reassigned' && $data['status']!='archived') && $user->user_level==User::ROLE_ADMIN ) :?>
												<a href="<?=Url::to(['erp-lpo-request-approval/reassign','lpo_request'=>$id,'f_id'=>$data['id']])?> "  
												class="btn-reassign" title="Reassign Approval" >
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_lpo_request_approval_flow as f where f.id > {$id} and 	f.originator={$user} and f.lpo_request={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
