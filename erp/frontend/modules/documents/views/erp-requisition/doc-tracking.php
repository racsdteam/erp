...
<?php 
     ...
     $q="SELECT f.*  FROM  erp_requisition_approval_flow as f where f.pr={$id}  order by f.timestamp desc";
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
                   ...
                   $row7=getUserSendInfo($data['approver'],$data['id'],$data['pr']);
                   if(empty($row7))
                   {
...
function getUserInfo($user,$datime){
   ...
   $q6=" SELECT u.first_name,u.last_name,p.position FROM user as u  inner join erp_persons_in_position as pp  on u.user_id=pp.person_id
                          inner join erp_org_positions as p  on pp.position_id=p.id  where pp.person_id='".$user."' and pp.timestamp<='".$datime."' and pp.status=1 ";
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_requisition_approval_flow as f where f.id > {$id} and 	f.originator={$user} and f.pr={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
    var_dump($row);           
                                           return $row;
