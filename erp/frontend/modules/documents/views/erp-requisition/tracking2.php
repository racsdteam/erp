...
$this->title = 'Purchase Requisition Tracking';
...
<div class="card card-default ">
 <div class="card-header ">
   <h3 class="card-title"><i class="fa fa-binoculars"></i> Purchase Requisition History Tracking</h3>
 </div>
 <div class="card-body">
...
<?php 
     ...
     $q="SELECT f.*  FROM  erp_requisition_approval_flow as f where f.pr_id={$id}  order by f.timestamp desc";
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
                  ...
                  $row7=getUserSendInfo($data['approver'],$data['id'],$data['pr_id']);
                   if(empty($row7))
                   {
							...
							<div class="widget-box transparent">
								<div class="widget-header widget-header-small">
									<h5 class="widget-title smaller">
										...
										<span class="grey"> received the Purchase Requisition</span>
									</h5>
...
function getUserInfo($user,$datetime){
   ...
   $q6=" SELECT u.first_name,u.last_name,p.position FROM user as u  inner join erp_persons_in_position as pp  on u.user_id=pp.person_id
                          inner join erp_org_positions as p  on pp.position_id=p.id  where pp.person_id='".$user."' and pp.timestamp < '".$datetime."'";
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_requisition_approval_flow as f where f.id > {$id} and 	f.originator={$user} and f.pr_id={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
                                           return $row;
}
