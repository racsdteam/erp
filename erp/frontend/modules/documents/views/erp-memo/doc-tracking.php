...
<div class="card card-default ">
 <div class="card-header ">
   <h3 class="card-title"><i class="fa fa-binoculars"></i> Memo History Tracking</h3>
 </div>
 <div class="card-body">
...
<?php 
     ...
     $q="SELECT f.*  FROM  erp_memo_approval_flow as f where f.memo_id={$id}  order by f.timestamp desc";
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
                    ...
                    $row7=getUserSendInfo($data['approver'],$data['id'],$data['memo_id']);
                   if(empty($row7))
                   {
							...
							<div class="widget-box transparent">
								<div class="widget-header widget-header-small">
									<h5 class="widget-title smaller">
										...
										<span class="grey"> received Memo</span>
									</h5>
								...
								<div class="widget-body">
									<div class="widget-main">
										...
										<span class="text-warning">Is Copy ? : </span>
									 <?php if($data['is_copy']) :?>
	                                ...
	                                <h6 class="widget-title smaller">
										...
										<small class="text-grey"><?php echo $diff->format(' Yrs : %Y , M : %m , Dys : %d , hrs : %h ,min : %i ,sec : %s');
										//00 years 0 months 0 days 08 hours 0 minutes 0 second  ?></small>
									</h6>
										...
										<div class="widget-toolbox clearfix">
											...
											<div class="pull-right action-buttons">
												...
												<a href="<?=Url::to(['erp-memo-approval/redirect','memo_id'=>$id,'f_id'=>$data['id']])?> "  
...
function getUserInfo($user,$datetime){
   ...
   $q6=" SELECT u.first_name,u.last_name,p.position FROM user as u  inner join erp_persons_in_position as pp  on u.user_id=pp.person_id
                          inner join erp_org_positions as p  on pp.position_id=p.id  where pp.person_id='".$user."' and pp.timestamp < '".$datetime."' order by timestamp desc";
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_memo_approval_flow as f where f.id > {$id} and 	f.originator={$user} and f.memo_id={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
