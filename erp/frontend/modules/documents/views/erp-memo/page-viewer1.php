...
I 
 ...
 $ser_domain='https://rac.co.rw';
    ...
    $url=Url::to(['erp-memo/pdf-data','id'=>$model->id]);
    $full_path=$ser_domain.$url; 
  
  $q1=" SELECT c.categ as type FROM erp_memo_categ as c
 inner join erp_memo as m  on c.id=m.type where m.id={$model->id} ";
...
<div class="d-flex flex-sm-row flex-column">
 ...
 <figure>
  ...
  <figcaption>Memo For <?php echo  $row['type'] ?></figcaption>
</figure>
  ...
  <?php   
        ...
        ])->from('erp_memo_approval_flow as f ')->where(['f.memo_id' =>$model->id])->orderBy([
                'timestamp' => SORT_DESC,
