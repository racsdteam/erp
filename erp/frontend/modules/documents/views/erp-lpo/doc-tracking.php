...
<?php 
     ...
     $q="SELECT f.*  FROM erp_lpo_approval_flow  as f where f.lpo={$id}  order by f.timestamp desc";
		...
		<div class="row">
			<div class="col-xs-12 col-sm-12 ">
			    ...
			    <?php if(!empty($res)):?>
                  ...
                  $row7=getUserSendInfo($data['approver'],$data['id'],$data['lpo']);
                   if(empty($row7))
                   {
							...
							<div class="widget-box transparent">
								<div class="widget-header widget-header-small">
									<h5 class="widget-title smaller">
										...
										<span class="grey"> received LPO</span>
									</h5>
...
function getUserSendInfo($user,$id,$doc){
   ...
   $q="SELECT f.*  FROM  erp_lpo_approval_flow as f where f.id > {$id} and 	f.originator={$user} and f.lpo={$doc}";
     $com = Yii::$app->db->createCommand($q);
     $row = $com->queryone();
