...
$msg=  Yii::$app->session->getFlash('success');
 ...
 <div class="box-body"> 
     ...
     <div class="alert alert-warning alert-dismissible">
                ...
                <ul class="notice">
                    <li> Return :  Return  Travel Document e.g : request for correction,request for more info </li>
                     <li>Forward : Send   Travel Document to the next user for approval (For managers,Technicians & Officers)</li>
                     <li>Approve & Forward :  Approve   Travel Document and forward it (For Directors)</li>
                  </ul>
...
inner join  erp_travel_request_approval_flow  as f on f.originator=u.user_id 
inner join erp_persons_in_position as pp on u.user_id=pp.person_id 
inner join erp_org_positions as p on p.id=pp.position_id where f.tr_id='".$tr_id."' order by f.timestamp desc";
...
inner join erp_travel_request as f on f.created_by=u.user_id 
inner join erp_persons_in_position as pp on u.user_id=pp.person_id 
inner join erp_org_positions as p on p.id=pp.position_id where f.id='".$tr_id."'";
...
?>
    <input type="hidden" id="TrId" name="ErpTravelRequestApproval[tr_id]" value="<?php echo $tr_id ?>">
    <input type="hidden" id="Action" name="ErpTravelRequestApproval[action]" value="rfa">
...
?>
 ...
 <input type="hidden" id="TrId" name="ErpTravelRequestApproval[tr_id]" value="<?php echo $tr_id ?>">
   <input type="hidden" id="Action" name="ErpTravelRequestApproval[action]" value="cforward">
...
?>
     <input type="hidden" id="TrId" name="ErpTravelRequestApproval[tr_id]" value="<?php echo $tr_id ?>">
   <input type="hidden" id="Action" name="ErpTravelRequestApproval[action]" value="approve">
...
<div class="row">
    ...
    <div class="col-xs-12 ol-sm-12 col-md-6 col-lg-6">
     ...
     <div class="col-xs-12 ol-sm-12 col-md-6 col-lg-6">
       ...
       <?= $form->field($model, 'employee')->dropDownList([], 
	         ['prompt'=>'-Choose a employee-','class'=>['Select2 form-control select2'],'id'=>'emp-4' ,'multiple'=>'multiple',
			  'onchange'=>'
			'])->label('Employee Name (automatically filled in)'); ?>   
