...
class ErpLpoController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        'matchCallback' => function ($rule, $action) {
            ...
            'verbs' => [
                ...
                'actions' => [
                    'delete' => ['POST'],  'delete-attachement' => ['POST'],
                ],
            ],
        ];
    }
 ...
 if(!empty($id)) {
  ...
  Yii::$app->db->createCommand()
                      ->update('erp_lpo_approval_flow', ['status' =>'archived'], ['lpo' =>$id,'approver'=>$user,'status' =>'pending'])
                      ->execute();  
        ...
        if (Yii::$app->request->post()) {
          ...
          if(isset($_POST['ErpLpo'])){
           ...
           $model->attributes=$_POST['ErpLpo'];
            ...
            $files=$model->uploaded_file ;
           
            if(!empty( $files)){
             
               $temp_path = 'uploads/lpo/temp';
                ...
                foreach($files as $file){
                 ...
                 if($file->saveAs($temp_path .'/'.$file->name)) {
                     
                    $pdf->addPDF($temp_path .'/'.$file->name, 'all');  
 ...
 $path_to_lpo='uploads/lpo/'. $unification.'.pdf';
 $pdf->merge('file',  $path_to_lpo, 'P');
   
   $model->lpo_upload=$path_to_lpo;
   $model->file_name= $files[0]->name; 
   ...
   if($flag=$model->save()){
    ...
    foreach($files as $file){
                  ...
                  if(file_exists($temp_path .'/'.$file->name)){
                     unlink($temp_path .'/'.$file->name);
            }
     ...
     if($flag){
      ...
      Yii::$app->session->setFlash('success',"Purchase Order Created");   
           ...
           Yii::$app->session->setFlash('failure',"Purchase  Order could not be created !"); 
          }   
...
if (Yii::$app->request->isAjax) {
            ...
            if(!empty($files)){
               ...
               $pdf = new \LynX39\LaraPdfMerger\PdfManage; 
                ...
                foreach($files as $file){
                  ...
                  $file->saveAs($temp_path .'/'.$file->name);   
 ...
 //--------------unlink existing lpo upload
 if(file_exists($model->lpo_upload)){
                     unlink($model->lpo_upload);
            }
 ...
 if($flag=$model->save(false)){
      ...
      Yii::$app->session->setFlash('success',"Purchase Order Updated");
           ...
           Yii::$app->session->setFlash('failure',"Purchase  Order could not be updated!"); 
          } 
...
if (Yii::$app->request->isAjax) {
 ...
 public function actionDeleteAttachement($id)
    {
     
     $model=$this->findModel($id);
    
     if ($flag=file_exists($model->lpo_upload)) {
           
          $flag= unlink($model->lpo_upload);
